...
const Training: React.FC = () => {
  return (
    <section id="training" className="py-24 px-6 bg-podDark">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {TRAINING_OFFERINGS.map((course, idx) => (
            <div key={idx} className="bg-podDeepPurple/20 p-8 rounded-3xl border border-white/5 hover:border-podPurple/20 transition-all group flex flex-col h-full">
              <div className="flex justify-between items-start mb-6">
                <div className="w-12 h-12 rounded-xl bg-podDark border border-white/10 flex items-center justify-center">
                  <Terminal className="w-6 h-6 text-podCyan" />
                </div>
              ...
              <div className="space-y-3 mb-8">
                {course.highlights.map((h, i) => (
                  <div key={i} className="flex items-center gap-3 text-sm text-slate-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-podPurple" />
                    {h}
                  </div>
                ))}
              </div>
