...
const Experience: React.FC = () => {
  return (
    <section id="experience" className="py-24 md:py-32 px-4 sm:px-6 bg-podDark relative overflow-hidden">
      <div className="absolute top-1/2 left-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-podPurple/5 rounded-full blur-[100px] md:blur-[120px] -translate-x-1/2" />
            ...
            {EXPERIENCE.map((exp, idx) => {
              ...
              return (
                <div key={idx} className={`relative flex flex-col lg:flex-row items-start lg:items-center gap-6 lg:gap-0 ${isEven ? 'lg:flex-row-reverse' : ''}`}>
                  {/* Timeline Dot */}
                  <div className="absolute left-6 lg:left-1/2 top-0 w-3 h-3 md:w-4 md:h-4 rounded-full bg-podPurple border-2 md:border-4 border-podDark shadow-[0_0_15px_rgba(139,92,246,0.8)] z-20 lg:-translate-x-1/2 mt-8 lg:mt-0" />

                  {/* Content Card */}
                  <div className={`w-full lg:w-[45%] pl-16 lg:pl-0 group`}>
                    <div className="relative p-6 md:p-8 bg-podDeepPurple/30 border border-white/5 rounded-2xl md:rounded-3xl hover:border-podPurple/30 transition-all duration-500 backdrop-blur-sm">
                      <div className="absolute -top-3 md:-top-4 right-4 md:right-8 px-3 py-1 bg-podDark border border-white/10 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-podCyan shadow-xl">
                      ...
                      <div className="flex flex-col sm:flex-row items-start gap-4 md:gap-6 mb-6 md:mb-8">
                        <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 shadow-lg transition-transform group-hover:scale-110 group-hover:rotate-6 bg-podDark border border-white/10`}>
                          {Icon && <Icon className={`w-6 h-6 md:w-7 md:h-7 text-white`} />}
                        </div>
                        <div>
                          <h3 className="text-xl md:text-2xl font-bold text-white group-hover:text-podCyan transition-colors">{exp.title}</h3>
                          <p className="text-podPurple font-bold text-base md:text-lg mb-2">{exp.company}</p>
