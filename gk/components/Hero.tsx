  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center items-center px-4 sm:px-6 overflow-hidden pt-20">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-6xl pointer-events-none opacity-30 md:opacity-40">
        <div className="absolute top-0 left-0 w-64 h-64 md:w-96 md:h-96 bg-pod-purple/20 rounded-full blur-[100px] md:blur-[150px]" />
        <div className="absolute bottom-0 right-0 w-64 h-64 md:w-96 md:h-96 bg-pod-cyan/10 rounded-full blur-[100px] md:blur-[150px]" />
      </div>

      <div className="max-w-6xl w-full flex flex-col lg:flex-row items-center gap-8 md:gap-12 z-10 mb-24 md:mb-20">
        <div className="flex-1 text-center lg:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-pod-purple/10 border border-pod-purple/20 text-pod-purple text-[10px] md:text-xs font-bold uppercase tracking-widest mb-6 md:mb-8">
          ...
          <p className="text-base sm:text-lg md:text-2xl text-slate-400 max-w-2xl mb-8 md:mb-12 leading-relaxed font-light mx-auto lg:mx-0">
            ...
            <span className="text-white font-medium"> Azure, Windows Server 2022, VMware</span> and <span className="text-pod-cyan font-medium">Gen AI</span>.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center lg:justify-start items-center">
            ...
            <a href="#projects" className="w-full sm:w-auto px-8 md:px-10 py-4 md:py-5 bg-pod-deep-purple/50 hover:bg-pod-deep-purple backdrop-blur-md border border-white/10 text-white rounded-2xl font-bold text-base md:text-lg transition-all hover:scale-105 text-center">
              View Work
            </a>
          </div>
        </div>
          ...
          <div className="bg-[#0b1120] rounded-2xl border border-white/10 shadow-2xl overflow-hidden font-mono text-sm group">
            <div className="bg-white/5 px-4 py-3 flex items-center justify-between border-b border-white/5">
              <div className="flex gap-2">
                ...
                <div className="w-3 h-3 rounded-full bg-pod-gold/50" />
                <div className="w-3 h-3 rounded-full bg-pod-cyan/50" />
              </div>
              <span className="text-slate-500 text-xs">gautham_kamath --expert</span>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex gap-3">
                <span className="text-pod-purple">$</span>
                <span className="text-slate-300">fetch --profile "Gautham Kamath"</span>
              </div>
              <div className="pl-6 text-slate-400 border-l border-pod-purple/20 space-y-1">
                <p><span className="text-pod-cyan">Role:</span> Microsoft Certified Trainer (MCT)</p>
                <p><span className="text-pod-cyan">Core:</span> Azure, Windows Server 03-22</p>
                <p><span className="text-pod-cyan">Focus:</span> Hybrid Cloud & Virtualization</p>
                <p><span className="text-pod-cyan">GenAI:</span> LLM Deployment on Azure</p>
              </div>
              <div className="flex gap-3">
                <span className="text-pod-purple">$</span>
                <span className="text-slate-300 animate-pulse text-pod-purple">_</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {[...RUNNING_SERVICES, ...RUNNING_SERVICES].map((service, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3 md:gap-5 mx-6 md:mx-10">
                <Icon className="w-4 h-4 md:w-5 md:h-5 text-pod-purple opacity-60" />
      ...
      <div className="absolute top-20 right-10 animate-bounce [animation-duration:5s] opacity-10 md:opacity-20 pointer-events-none">
        <Sparkles className="w-12 h-12 md:w-16 md:h-16 text-pod-cyan" />
      </div>
    </section>
  );
};
